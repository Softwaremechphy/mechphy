...
export default function MapSection({ soldiers = [], selectedSoldierId, isReplayMode = false }) {
...
const soldierColors = [
  ...
  // Remove any local state for soldiers, always use the prop
    ...
    const timer = setTimeout(() => {
      ...
      try {
        mapRef.current = L.map(container, {
          ...
          minZoom: 10,
          maxZoom: 21,
          preferCanvas: false, // Canvas can cause lag with many tiles
          zoomControl: true,
        ...
        class MBTilesLayer extends L.TileLayer {
          ...
          // Override createTile to set crossOrigin and fix tile seams
        ...
        mbTilesLayerRef.current = new MBTilesLayer({
          ...
          errorTileUrl:
            "data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAAC0lEQVQIHWNgAAIAAAUAAY27m/MAAAAASUVORK5CYII=",
          bounds: null,
          noWrap: true, // Prevent wrapping artifacts
          keepBuffer: 8, // Keep more tiles in memory to reduce white patches
          updateWhenIdle: false,
          updateWhenZooming: false, // Reduce lag on zoom
          crossOrigin: true,
          detectRetina: false,
          edgeBufferTiles: 2,
        });
      {/* Extra CSS to fix tile seams and background, and force seamless rendering */}
      <style jsx global>{`
        .leaflet-tile {
      ...
      `}</style>
      {/* Extra CSS to fix tile seams and background */}
      <style jsx global>{`
        .leaflet-tile {
          image-rendering: pixelated !important;
  ...
  return (
    <div className="map-container" style={{ position: "relative", height: "100%", width: "100%" }}>
      ...
      {isReplayMode && (
        <div className="aar-mode-indicator">
          AAR MODE
        </div>
      )}
      ...
      {showControls && !mapLoaded && (
        <div className="scifi-panel">
          ...
          <div className="scifi-title">{isReplayMode ? 'SELECT AAR MAP' : 'SELECT MAP'}</div>
